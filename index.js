...
const authmethods = {
    // only two methods of authentication are supported
    NOAUTH: 0,
    USERPASS: 2
}
...
let autherHandler = function (data) {
    ...
    if(VERSION !== 5) {
        // other versions of socks protocol are not supported
...
let requestHandler = function (data) {
    ...
    let CMD = data [1]; // 0x01 supports connect connection first
        if(cmd !== 1)
          console.error('Other connection% d is not supported', CMD);
